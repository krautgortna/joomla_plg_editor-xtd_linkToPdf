...
elseif($override == 'pdflink'){
	$onClick = "createPdfTag();window.parent.jModalClose();";
}
...
<script>
function createPdfTag(){
    var editor = ImageManager.editor
    var url    = ImageManager.fields.url.value
    var link_text = document.getElementById("link_text").value
    var tag = '<a target="_blank" href="'+ url +'">' + link_text + '</a>'
...
//upFolder Hack:
function upFolder() {
    var path = ImageManager.folderlist.value.split('/'), search;
    path.pop();
    search = path.join('/');
    ImageManager.setFolder(search);
    <?php if($override == 'pdflink') : ?>
      setFrameUrl(search, ImageManager.asset, ImageManager.author)
    <?php else: ?>
      ImageManager.setFrameUrl(search, ImageManager.asset, ImageManager.author)
    <?php endif; ?>
}

function setFrameUrl(folder, asset, author) {
    var qs = {
        option: 'com_media',
        view: 'imagesList',
        tmpl: 'component',
    ...
    ImageManager.frameurl = 'index.php?option=com_media&view=imagesList&tmpl=component&asset=' + asset + '&author=' + author + '&override=pdflink' + '&folder=' + folder;
    ImageManager.frame.location.href = ImageManager.frameurl
}
</script>
...
<div class="container-popup">
	...
	<form action="index.php?option=com_media&amp;asset=<?php echo $asset; ?>&amp;author=<?php echo $author; ?>" class="form-vertical" id="imageForm" method="post" enctype="multipart/form-data">
			...
			<div class="row">
				<div class="span6 control-group">
					...
					<div class="controls">
						...
						<button class="btn" type="button" id="upbuttonPDFmod" onclick="upFolder();" title="<?php echo JText::_('COM_MEDIA_DIRECTORY_UP'); ?>"><?php echo JText::_('COM_MEDIA_UP'); ?></button>
					</div>
				</div>
		...
		<iframe id="imageframe" name="imageframe" src="index.php?option=com_media&amp;view=imagesList&amp;tmpl=component&amp;folder=<?php echo rawurlencode($this->state->folder); ?>&amp;asset=<?php echo $asset; ?>&amp;author=<?php echo $author; ?><?php if($override == 'pdflink') echo "&amp;override=pdflink" ?>" ></iframe>
  ...
  <?php if($override == 'pdflink') : ?>
    <div class="well">
        ...
        <div class="span6 control-group">
					...
					<div class="controls">
						<input type="text" id="link_text" value="Download PDF" />
					</div>
				</div>
			</div>
